...
export function QuotationForm({ products, customers, taxRate, currency, lakhCroreFormat }: QuotationFormProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const parsed = createQuotationSchema.safeParse({
      customerId,
      customerNameFree: customerId ? "" : customerNameFree,
      items,
    ...
    setSubmitting(true);
    try {
      const { id } = await createQuotation(parsed.data);
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-6">
      <Card className="flex flex-col gap-4">
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div className="flex flex-col gap-2">
            <Label htmlFor="quoteCustomer">Saved customer (optional)</Label>
            <Select id="quoteCustomer" value={customerId} onChange={(e) => setCustomerId(e.target.value)}>
              <option value="">None — use free-text name below</option>
              {customers.map((c) => (
          ...
          <div className="flex flex-col gap-2">
            <Label htmlFor="quoteCustomerFree">Customer name (if not saved above)</Label>
            <Input
              id="quoteCustomerFree"
              value={customerNameFree}
              onChange={(e) => setCustomerNameFree(e.target.value)}
              disabled={!!customerId}
        ...
        <div className="flex flex-col gap-2">
          <Label htmlFor="quoteProductSearch">Add product</Label>
          <div className="relative">
            <Input
              id="quoteProductSearch"
      ...
      <Card className="flex flex-col gap-4">
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div className="flex flex-col gap-2">
            <Label htmlFor="quoteBillDiscount">Bill discount</Label>
            <Input id="quoteBillDiscount" inputMode="decimal" value={billDiscount} onChange={(e) => setBillDiscount(e.target.value)} />
          </div>
          <div className="flex flex-col gap-2">
        ...
        <div className="flex flex-col gap-2">
          <Label htmlFor="quoteNote">Note (optional)</Label>
          <Input id="quoteNote" value={note} onChange={(e) => setNote(e.target.value)} />
        </div>
