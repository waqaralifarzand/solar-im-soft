...
export function ReceivePaymentDialog({ customerId, currentBalance }: ReceivePaymentDialogProps) {
  ...
  const { fieldErrors, validateOnSubmit, validateField, clearErrors } = useZodFormErrors(receivePaymentSchema);
  ...
  const balVal = Number(currentBalance);
      ...
      <DialogContent>
        ...
        {balVal > 0 && (
          <p className="mb-2 text-sm text-muted-foreground">
            Outstanding balance: <span className="font-medium text-destructive">{currentBalance}</span>
          </p>
        )}
